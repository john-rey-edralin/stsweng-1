...
const form_functions = {
    ...
    validDate: (dateInput, dateMin) => {
        ...
        if (dateInput.length > 10) return false;
            ...
            else if (input - min < 0 || isNaN(input)) return false;
            else if (input - max >= 0 || isNaN(input)) return false;
            //true = valid date
            else return true;
        }
    },
    ...
    validName: (nameInput) => {
        ...
        else if (checkStringInput(name)) return false;
        else return true;
    },

    validNumber: (numInput) => {
        if (validator.isEmpty(numInput)) return false;
        else if (numInput.length === 11) return true;
        else return false;
    },

    validEventType: (eTypeInput) => {
        var eventtype = validator.trim(eTypeInput);
        if (validator.isEmpty(eventtype)) return false;
        return true;
    },

    validEventTime: (eTimeInput) => {
        if (validator.isEmpty(eTimeInput)) return false;
        return true;
    },

    validPaxNum: (paxNumInput) => {
        if (paxNumInput < 0) return false;
        else if (paxNumInput == 0) return false;
        else if (paxNumInput > 120) return false;
        return true;
    },
    ...
    validPackage: (garden, sunroom, terrace) => {
        ...
        if (eventpackage == 0) return false;
        else return true;
    },
